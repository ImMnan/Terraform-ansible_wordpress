/* Start with adding the local variables, which will be used throughout the script ami-id of the instance (see [2.5]),
 use VPS (see [2.3]), key_name (see [2.2]) and key_path (see [2.2]) */
...
(Or any cloud provider you are using)(See [2.4]) */
...
/* Creating a security group with the name of wpaccess and setting ingress egress security rules, it will automatically use the vac id from variables declared in local. */
